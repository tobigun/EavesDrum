...
static void loadExcludeList(FS& fs, const char *filename)
{
...
AsyncFSEditor::AsyncFSEditor(const String& username, const String& password, const FS& fs)
  : _fs(fs)
  , _username(username)
  , _password(password)
...
bool AsyncFSEditor::canHandle(AsyncWebServerRequest *request)
{
  if (request->url().equalsIgnoreCase("/edit"))
  {
    if (request->method() == HTTP_GET)
    {
      ...
      {
        request->_tempFile = _fs.open( (request->arg("edit")).c_str(), FILE_READ);
      ...
      {
        request->_tempFile = _fs.open( (request->arg("download")).c_str(), FILE_READ);
...
void AsyncFSEditor::handleRequest(AsyncWebServerRequest *request)
{
  ...
  if (request->method() == HTTP_GET)
  {
    ...
    else
    {
      ...
      {
        DEBUGF("AsyncFSEditor::handleRequest: Sending AsyncWebServerResponse");
  ...
  else if (request->method() == HTTP_DELETE)
  {
    ...
    {
      _fs.remove( (request->getParam("path", true)->value()).c_str() );
  ...
  {
    if (request->hasParam("data", true, true) && _fs.exists( (request->getParam("data", true, true)->value()).c_str()))
      request->send(200, "", ("UPLOADED: " + request->getParam("data", true, true)->value()).c_str());
    else
      request->send(500);
  }
  else if (request->method() == HTTP_PUT)
  {
    if (request->hasParam("path", true))
    {
      ...
      if (_fs.exists(filename.c_str()))
      {
        request->send(200);
      }
      else
      {
        File f = _fs.open(filename.c_str(), FILE_WRITE);
...
void AsyncFSEditor::handleUpload(AsyncWebServerRequest *request, const String& filename, size_t index,
                                 uint8_t *data, size_t len, bool final)
{
  if (!index)
  {
    if (!_username.length() || request->authenticate(_username.c_str(), _password.c_str()))
    {
      ...
      request->_tempFile = _fs.open(filename.c_str(), FILE_WRITE);
